...
if($ty=='deletevoteuser'){
	...
	//查询所删除的视频是否存在
	if(!empty($video) && $video['soft_delete'] == 0){
		//软删除该条视频数据
		pdo_update('tyzm_diamondvote_votevideo',array('soft_delete'=>1), array('num' => $id,'uniacid' => $uniacid,'rid' => $rid));
		//删除相关的投票数据
		pdo_update($this->tablevotedata, array('soft_delete'=>1), array('tid' => $id,'rid' => $rid,'uniacid' => $uniacid));
		//删除相关的统计数据
		pdo_update($this->tablecount, array('soft_delete'=>1), array('tid' => $id,'rid' => $rid,'uniacid' => $uniacid));
