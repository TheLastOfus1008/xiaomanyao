...
defined('IN_IA') or exit('Access Denied');
		...
		if (!empty($list)) {
			 ...
			 foreach ($list as &$item){             
				...
				$item['votetotal'] = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename($this->tablevotedata) . " WHERE   rid = :rid AND votetype=0 AND soft_delete=0", array(':rid' => $item['rid']));
				...
				$item['pvtotal']=pdo_fetchcolumn("SELECT sum(pv_total) FROM ".tablename($this->tablecount)." WHERE rid = :rid AND soft_delete=0", array(':rid' => $item['rid']));
				$item['pvtotal']=$item['virtualpvtotal']+$item['pvtotal'];

				$item['sharetotal']=pdo_fetchcolumn("SELECT sum(share_total) FROM ".tablename($this->tablecount)." WHERE rid = :rid AND soft_delete=0", array(':rid' => $item['rid']));
				...
				$item['videocount'] = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('tyzm_diamondvote_votevideo') . " WHERE   rid = :rid AND soft_delete=0", array(':rid' => $item['rid']));
