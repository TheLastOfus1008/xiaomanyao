...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		$resp = ihttp_request('#/gateway.php/a=rr&b=check', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request('#/gateway.php/a=rr&b=download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
